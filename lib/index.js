  ...
  reader
      ...
      if (file === "package.json") {
        ...
        if (args.onlyDirectAndDevDependencies) {
          ...
          if (packageJsonContents.name) {
            ...
            onlyDirectDependenciesFilter[packageJsonContents.name] = packages;
          }
        }
      }
      reader.next();
    ...
    .on("done", function(error) {
      if (!error) {
        async.eachSeries(
          filePaths,
          function(filePath, iteratorCallback) {
              ...
              Object.keys(licenses)
                ...
                .forEach(function(item) {
                  ...
                  if (license) {
                    ...
                    if (
                      args.onlyDirectDependencies ||
                      args.onlyDirectAndDevDependencies
                    ) {
                      if (
                        !onlyDirectDependenciesFilter[license.parents] ||
                        onlyDirectDependenciesFilter[license.parents].indexOf(
                          packageName
                        ) == -1
                      ) {
                        return;
